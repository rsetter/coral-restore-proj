...
#load empirical data ("reference")
oaemp05 <- read.csv(file="E:/oa/empirical/emp_2005_suit.csv",row.names=NULL,header=F,skip=1,colClasses=c(rep("NULL",2),"numeric","numeric","NULL","factor"))
...
#load empirical data ("reference)
popemp05 <- read.csv(file="E:/pop_hr/hist_gpw/histgpw_2005_suit.csv",row.names=NULL,header=T,
                      colClasses=c(rep("NULL",2),rep("numeric",2),rep("NULL",3),"factor"))
names(popemp05) <- c("iscoral","ID","suitemp")
...
#from https://www.reddit.com/r/rstats/comments/c6lvg0/confusion_matrix_caret_plotting_superior_to_base/
