...
empirical <- read.csv("E:/oa/co2sys_input/empirical/empirical_2005_co2sysout.csv")
...
#taylor diagram of just coral sites
  ...
  emp2005 <- read.csv(file="E:/oa/empirical/emp_2005_suit.csv",row.names=NULL,header=F,skip=1,colClasses=c("numeric",rep("NULL",2),"numeric",rep("NULL",5)))
    names(emp2005) <- c("pointid","empirical")
  ...
  for(i in 1:length(rasterList)){
    r<-rasterList[[i]]
    reefext <- rgis::fast_extract(sf=reef,ras=r,funct=function(x){max(x,na.rm=T)},parallel=T)
  ...
  reefext <- rgis::fast_extract(sf=reef,ras=modelm,funct=function(x){max(x,na.rm=T)},parallel=T)
  dfext <- as.data.frame(reefext)
  mndf <- dfext[,c(1,3)]
  names(mndf) <- c("pointid","meanmod")
  
  modelmed <- raster("E:/oa/co2sys_input/historical/OAhist_modelmedian.tif",band=16)
    reefext <- rgis::fast_extract(sf=reef,ras=modelmed,funct=function(x){max(x,na.rm=T)},parallel=T)
  dfext <- as.data.frame(reefext)
  medf <- dfext[,c(1,3)]
...
alldata <- join_all(list(emp2005,mod1,mod2,mod3,mod4,mod5,mod6,mod7,mod8,mod9,mod10,mndf,medf,filter),by=c("pointid"))
