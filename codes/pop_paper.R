...
ssp1suitcsvout <- paste("ssp1_",2000+(1:nssp1files+1)*5,"_suit.csv",sep="")
...
for (i in 1:nssp1files){
  ...
  write.csv(popdf,ssp1suitcsvout[i], row.names=FALSE)
...
for (i in 1:ncsvfiles){
  ...
  y <- add_column(c,year=paste0("20",(i+1)*5),.before=T)
  count <- rbind(y,count)
} 
...
ssp2suitcsvout <- paste("ssp2_",2000+(1:nssp2files+1)*5,"_suit.csv",sep="")
...
for (i in 1:nssp2files){
  ...
  poppt <- inner_join(reef,result,by="pointid")
  popdf <- as.data.frame(poppt)
  write.csv(popdf,ssp2suitcsvout[i], row.names=FALSE)
...
for (i in 1:ncsvfiles){
  ...
  b$pop_coral <- b$suit * b$iscoral
  c <- count(b, vars="pop_coral")
  y <- add_column(c,year=paste0(2000+(i+1)*5),.before=T)
  count <- rbind(y,count)
} 
...
ssp3suitcsvout <- paste("ssp3_",2000+(1:nssp3files+1)*5,"_suit.csv",sep="")
...
for (i in 1:nssp3files){
  ...
  poppt <- inner_join(reef,result,by="pointid")
  popdf <- as.data.frame(poppt)
  write.csv(popdf,ssp3suitcsvout[i], row.names=FALSE)
...
ssp5suitcsvout <- paste("ssp5_",2000+(1:nssp5files+1)*5,"_suit.csv",sep="")
...
for (i in 18:nssp5files){
  ...
  poppt <- inner_join(reef,result,by="pointid")
  popdf <- as.data.frame(poppt)
  write.csv(popdf,ssp5suitcsvout[i], row.names=FALSE)
