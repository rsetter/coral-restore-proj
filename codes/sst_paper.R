...
modelm <- overlay(m1e,m2,m3,m4,m5,m6,m7,m8d,m9,m10d,m11,m12e,m13e,m14e,m15e,m16,m17,m18,m19d,m20,m21,m22,m23e,m24,fun=function(x){ mean(x,na.rm=T)},
                  filename="E:/sst/SSThist_modelmean.tif")
modelmed <-overlay(m1e,m2,m3,m4,m5,m6,m7,m8d,m9,m10d,m11,m12e,m13e,m14e,m15e,m16,m17,m18,m19d,m20,m21,m22,m23e,m24,fun=function(x){ median(x,na.rm=T)},
...
empiricalp <- projectRaster(empirical,m2,method="bilinear")
empd <- dropLayer(empiricalp,c(290:450))
empdm<- calc(empd,fun=mean,na.rm=T)
empext <- rgis::fast_extract(sf=reef,ras=empdm,funct=function(x){max(x,na.rm=T)},parallel=T)
empdfext <- as.data.frame(empext)
empdf <- empdfext[,c(1,3)]
names(empdf) <- c("pointid","oisst")



##get mean values for each model, model mean, model median for same test period - 1981-2005
##extract only values at coral points
rasterList <- list(m1e,m2,m3,m4,m5,m6,m7,m8d,m9,m10d,m11,m12e,m13e,m14e,m15e,m16,m17,m18,m19d,m20,m21,m22,m23e,m24)
for(i in 1:length(rasterList)){
  r<-rasterList[[i]]
  rd <- dropLayer(r,c(1:1583))
  rm<- calc(rd,fun=mean,na.rm=T)
  reefext <- rgis::fast_extract(sf=reef,ras=rm,funct=function(x){max(x,na.rm=T)},parallel=T)
  dfext <- as.data.frame(reefext)
  dfext$SSTC <- dfext$rm_layer-273.15
  rdf <- dfext[,c(1,5)]
  names(rdf) <- c("pointid",paste("model",i))
  a <- paste("mod", i, sep = "")
  assign(a,rdf)
}
...
filter <- read.csv(file="E:/currentcoral/filter.csv",row.names=NULL,header=T,colClasses=c("numeric",rep("NULL",5),"numeric","NULL","NULL","NULL"))

alldata <- join_all(list(filter,empdf,mndf,medf,mod1,mod2,mod3,mod4,mod5,mod6,mod7,mod8,mod9,mod10,mod11,mod12,mod13,mod14,mod15,
                         mod16,mod17,mod18,mod19,mod20,mod21,mod22,mod23,mod24),by=c("pointid"))
names(alldata) <- c("pointid","iscoral","oisst","meanmod","medianmod","model1","model2","model3","model4","model5","model6","model7","model8","model9","model10",
                    "model11","model12","model13","model14","model15","model16","model17","model18","model19","model20","model21","model22","model23","model24")
...
oldpar<-taylor.diagram(alldf$oisst,alldf$model19,add=TRUE,normalize=TRUE,sd.arcs=TRUE,pcex=1.5,pch=19,col="black")
...
sst <- brick("E:/sst/SSThist_modelmedian.tif")
...
sst <- brick("E:/sst/ssthistmed_mean10.tif")
reefs <- st_read("E:/currentcoral/reefs.shp")
...
dfext <- as.data.frame(reefext)
for (i in 1:16){
  x <- paste0("sst_ssthistmed_mean10.",i)
  dfi <- subset(dfext,select=c("pointid","grid_code", "iscoral","ID",x))
  names(dfi) <- c("pointid","grid_code", "iscoral","ID","sst")
  dfi$suit <- with(dfi, ifelse(sst >= 297, ifelse(sst <= 305,1,0), 0)) #in kelvin. 24 to 32C. 297 to 305K
